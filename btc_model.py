...
# 🏢 官方 mNAV 專屬資本結構固定參數 (2026/06/15 官方數據基準，單位：美元)
MSTR_DEBT = 6754000000.0            # 🎯 總負債 Debt ($M)
MSTR_PREFERRED = 15475000000.0      # 🎯 特別股 Pref ($M)
MSTR_CASH = 1100000000.0            # 🎯 美元現金 USD Reserve ($M)
...
if ticker_data is not None:
    ...
    # s7: MSTR 靜態 mNAV 現貨溢價指標 [雙重指標核算] (滿分 15)
    if mstr_live_price is not None:
        # A. 原本「去泡沫現貨溢價」算法 (約 0.90倍，股價/每股現貨幣值)
        estimated_nav = (btc_price * BTC_PER_SHARE) 
        mstr_premium_rate = mstr_live_price / estimated_nav if estimated_nav > 0 else 0.90
        
        # B. 官方定義 mNAV 算法 (動態企業價值 EV / 比特幣總市值 BTC NAV)
        # 1. 股票市值 (Market Cap) = 即時股價 * 流通股數
        current_mcap = mstr_live_price * MSTR_SHARES_OUTSTANDING
        # 2. 官方企業價值分子：EV = 市值 + 債務 + 特別股 - 現金
        dynamic_ev = current_mcap + MSTR_DEBT + MSTR_PREFERRED - MSTR_CASH
        # 3. 官方分母：BTC NAV = 持有總量 * 即時 BTC 價格
        official_btc_nav = MSTR_BTC_HOLDINGS * btc_price
        # 4. 導出官方純動態 mNAV 倍數
        official_mnav_rate = dynamic_ev / official_btc_nav if official_btc_nav > 0 else 1.22
        ...
        st.markdown(f"""
            ...
            <div class="cute-card">
                ...
                <div class="cute-value">
                    ...
                    <span style="color:#7f8c8d; font-size:13px;">💡 目前已改採官方最新控制台公式（EV / BTC總價值 = {official_mnav_rate:.2f} 倍）。數字越低泡沫越小，對我們越安全！</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
