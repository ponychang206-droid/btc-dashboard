...
# 📊 核心配置：精確對齊最新公佈數據基準
MSTR_BTC_HOLDINGS = 846842          # 🎯 最新總持倉
MSTR_SHARES_OUTSTANDING = 386052000 # 🎯 最新基本流通股數 (ADSO)
MSTR_AVG_COST = 75656               # 🎯 最新平均買入成本

# 🏢 官方 mNAV 專屬資本結構參數 (完全對齊官方定義公式，單位：美元)
MSTR_DEBT = 6754000000.0            # 🎯 總負債 (Debt)
MSTR_PREFERRED = 15475000000.0      # 🎯 名目特別股價值 (Preferred Stock)
MSTR_CASH = 1100000000.0            # 🎯 現金儲備 (Cash)

BTC_PER_SHARE = MSTR_BTC_HOLDINGS / MSTR_SHARES_OUTSTANDING  # 自動核算動態每股含幣量
...
if ticker_data is not None:
    ...
    # s7: MSTR 官方 mNAV 溢價指標核心計算模組 (滿分 15)
    if mstr_live_price is not None:
        # A. 去泡沫現貨溢價視角 (純股票市值 / 純幣值)
        ...
        # B. 完全對齊官方定義 mNAV 算法 (EV / BTC NAV)
        # 1. 計算股票市值 (Market Cap)
        current_market_cap = mstr_live_price * MSTR_SHARES_OUTSTANDING
        # 2. 精確執行官方公式分子：EV = Market Cap + Debt + Preferred Stock - Cash
        official_ev = current_market_cap + MSTR_DEBT + MSTR_PREFERRED - MSTR_CASH
        # 3. 精確執行官方公式分母：BTC NAV = 持有總顆數 * 即時現貨價格
        official_btc_nav = MSTR_BTC_HOLDINGS * btc_price
        # 4. 導出官方純粹 mNAV 倍數
        official_mnav_rate = official_ev / official_btc_nav if official_btc_nav > 0 else 1.21
        
        # 評分權重採用官方 mNAV 作為核心防線
        s7 = max(0.0, min(15.0, ((2.5 - official_mnav_rate) / 1.5) * 15.0))
    else:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 📊 MSTR 官方資本結構參數清單")
    st.info(f"🪙 持倉總量: {MSTR_BTC_HOLDINGS:,} BTC")
    st.info(f"📜 基本流通股數: {MSTR_SHARES_OUTSTANDING:,} 股")
    st.info(f"📉 歷史總平均成本: ${MSTR_AVG_COST:,} USD")
    st.info(f"🏦 總債務 (Debt): ${MSTR_DEBT/1e9:.2f} B USD")
    st.info(f"🎀 名目特別股: ${MSTR_PREFERRED/1e9:.2f} B USD")
    st.info(f"💵 現金儲備 (Cash): ${MSTR_CASH/1e9:.2f} B USD")
...
if page == "直男量化經理人版":
    ...
    st.markdown(f"""
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #2b3139; margin-bottom: 25px;">
            <div style="font-size: 24px; font-weight: 700; color: #eaecef; display: flex; align-items: center; gap: 10px;">
                <span style="color: #f3ba2f;">🔮</span> BTC 9 因子抄底監控看板 (官方精確 mNAV 核算版)
            </div>
    ...
    with col_right:
        ...
        st.markdown(f"""
            ...
            <div class="metric-card"><div class="metric-title"><span>【美股風向球】MSTR 官方 mNAV 企業價值溢價指標 [s7] (權重: 15%)</span> {get_ui_badge(s7, 15.0)}</div><div class="metric-value">{s7:.1f} / 15.0 分 <span style="font-size:12px; color:#848e9c; font-weight:normal; margin-left:10px;">官方 mNAV: {official_mnav_rate:.2f} 倍 | 現貨價折溢價: {mstr_premium_rate:.2f} 倍</span></div></div>
    ...
    for p, name in zip(sim_btc_prices, sim_scenarios):
        ...
        # 清算價值 = 比特幣價值 - 債務 - 特別股 + 現金 (完全對齊官方資產扣除邏輯)
        sim_nav_total = sim_total_btc_value - MSTR_DEBT - MSTR_PREFERRED + MSTR_CASH
        ...
        rows.append({
            "情境說明": name,
            "比特幣模擬現貨價 (USD)": f"${p:,.0f}",
            "每股清算淨值 (NAV 地板價)": f"${sim_nav_per_share:,.2f}" if sim_nav_per_share > 0 else "$0.00",
            "1.2 倍溢價 (合理防線/抄底點)": f"${floor_price:,.2f}" if floor_price > 0 else "$0.00",
            "1.8 倍溢價 (牛市常態/泡沫線)": f"${ceiling_price:,.2f}" if ceiling_price > 0 else "$0.00"
        })
    ...
    st.markdown("""
        <div style="background-color: #181a20; border: 1px solid #2b3139; border-radius: 8px; padding: 20px; margin: 15px 0;">
            ...
            <p style="color: #ffffff; font-size: 14px; line-height: 1.6; margin-bottom: 14px;">
                本數據採用最嚴格的<b>「完全稀釋（Fully Diluted）」模型</b>。核心邏輯：將未來所有可轉債與特別股權益視為全部轉股（分母極大化至 4 億股），這能為策略構築出最保守、安全的防守邊界。
            </p>
            <ul style="color: #ffffff; font-size: 14px; line-height: 1.7; padding-left: 20px;">
                <li style="margin-bottom: 10px;">
                    <b>1. 抓出「極端防守地板價」（看 1.2 倍溢價欄位）</b>：當市場回歸理性，MSTR 溢價率修正到 1.2 倍附近。若 BTC 在 10 萬美元，排除所有潛在轉股稀釋後，MSTR 跌到 <span style="color: #f3ba2f; font-weight: bold;">$256</span> 附近就是鐵板支撐。
                </li>
                <li style="margin-bottom: 10px;">
                    <b>2. 判斷「天花板與泡沫警戒」（看 1.8 倍溢價欄位）</b>：當市場狂熱，華爾街給到 1.8 倍高溢價。在 BTC 10 萬美元時，MSTR 觸及 <span style="color: #f3ba2f; font-weight: bold;">$383</span> 附近即代表即使在完全稀釋的預期下，也已把未來漲幅預支完畢。
                </li>
            </ul>
        </div>
    """, unsafe_allow_html=True)

    with st.expander("🔬 🔍 全套 9 因子量化核心指標定義與數據來源說明書（官方精確對齊版）"):
        st.markdown("""
        ### 📊 核心加權計分邏輯說明
        ...
        #### **1. 【美股風向球】MSTR 官方 mNAV 企業價值溢價指標 [s7]**
        * **因子定義**：完全遵循 Saylor 官方 8-K 定義。分子採用企業總價值（EV = 市值 + 總負債 + 特別股 - 現金儲備），分母為（BTC 總持倉 * 即時現貨價），杜絕因單位換算或常數重疊導致的計算扭曲。
        * **數據來源**：`yfinance` 即時報價與微策略官方財務披露核算。
        """)
...
# ==========================================
    ...
    with col_right:
        ...
        st.markdown(f"""
            ...
            <div class="cute-card">
                <div class="cute-title"><span>🤡 美股大企業有沒有吹大泡泡 [s7]</span> {get_cute_badge(s7, 15.0)}</div>
                <div class="cute-value">
                    <b>特價得分：{s7:.1f} / 15.0 滿分</b><br>
                    <span style="color:#7f8c8d; font-size:13px;">💡 目前已精確同步官方 mNAV 公式核算（當前官方溢價率：{official_mnav_rate:.2f} 倍）。數字越低泡沫越小，對我們越安全！</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
