...
MSTR_BTC_HOLDINGS = 845256          # 最新總持倉
MSTR_SHARES_OUTSTANDING = 382756000 # 🎯 回歸最嚴苛基本流通股數（扣除未轉換債務、特別股）
MSTR_AVG_COST = 75680               # 歷史總平均買入成本
BTC_PER_SHARE = MSTR_BTC_HOLDINGS / MSTR_SHARES_OUTSTANDING  # 自動核算動態每股實打實含幣量 (~0.00220834 BTC)
...
if page == "直男量化經理人版":
    ...
    st.markdown("基於最新 845,256 BTC 持倉，**模擬當未來債主全部換成股票（全面稀釋總股數膨脹至 4 億股）**時，在不同 BTC 幣價情境下 MSTR 的股價合理風控區間：")
    ...
    **操盤風控指南 —— 怎麼看這份測試數據？**
    ...
    本數據採用最嚴格的「完全稀釋（Fully Diluted）」模型。核心邏輯：將未來所有可轉債視為全部轉股（分母極大化），因此反推估算出的「每股合理價」會被稀釋壓低，這能為策略構築出最保守、安全的防守邊界。
    
    1. **抓出「極端防守地板價」（看 1.2 倍溢價欄位）**：當市場回歸理性，MSTR 溢價率修正到 1.2 倍附近。若 BTC 在 10 萬美元，排除所有潛在轉股稀釋後，MSTR 跌到 **$256** 附近就是鐵板支撐。這是用來「左側分批抄底」或「大波段動態停損」的終極防線，比常規 ADSO 算出的價格更低、容錯率更高。
    2. **判斷「天花板與泡沫警戒」（看 1.8 倍溢價欄位）**：當市場狂熱，華爾街給到 1.8 倍高溢價。在 BTC 10 萬美元時，MSTR 觸及 **$383** 附近即代表即使在完全稀釋的預期下，也已把未來漲幅預支完畢。此時若股價繼續飆升，意味著真實泡沫率嚴重過高，切勿盲目追多，需防範多頭踩踏的暴跌修正。
    3. **稀釋效應的「鈍化現象」**：市場最擔心的「瘋狂發債、股本膨脹」，在 MSTR 模式下會被幣價上漲給鈍化。數據顯示，即便可轉債全數轉股（分母變大），只要 BTC 幣價能一路震盪上行（如到 15 萬美元），完全稀釋下的 1.2 倍地板價仍會被暴力拉升到 **$319**。長期來看，資產增速大於股本稀釋速度，上行空間就不會被鎖死。
    """
        ...
        * **因子定義**：**此指標採用動態「基本流通股數」風控標準。** 完美對齊最新官方基本股數 3.82756 億股結構，移除未轉換可轉債的潛在泡沫分母。當溢價率壓縮到 1.1x~1.2x 區間時，代表美股溢價泡沫已降至健康水位，適合擴大現貨槓桿。
